...
// Enhanced method to check wallet connection and fetch NFTs with retries
  ...
  createNFTDisplayBoard(
    ...
    const createNFTTexture = (): Promise<THREE.Texture> => {
      ...
      // Clear background with semi-transparent dark background
      ...
      return new Promise<THREE.Texture>((resolve) => {
        ...
        const drawImages = () => {
          ...
          loadedImages.forEach((img, index) => {
            ...
            const imageX = col * (imageSize + padding) + (canvas.width - (maxImagesPerRow * (imageSize + padding))) / 2;
            const imageY = row * (imageSize + padding) + 50;
